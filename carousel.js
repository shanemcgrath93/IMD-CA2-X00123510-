...
$('document').ready(function () {
	...
	$('.navscroll').on('mouseover', function() {
		if (this.id == 'nav-left') {
			if (currentSection != 1) $(this).addClass('navscroll-hover');
		} else {
			if (currentSection+1 <= sections) $(this).addClass('navscroll-hover');
		}
	}).on('mouseout', function () {
		$(this).removeClass('navscroll-hover');
	});
...
function setCurrentSection() {
	...
	$('#nav-left').removeClass('navscroll-hover');
	$('#nav-right').prop('disabled', false);
	$('#nav-right').removeClass('navscroll-hover');
}
